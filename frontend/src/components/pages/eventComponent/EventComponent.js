...
export default function EventComponent({ event, sameEvents, user, funcs }) {
    ...
    const addToFavorites = () => {
        if (user.isAuth && !block) {
            ...
            if (isFavorite) {
                setIsFavorite(false);
                authDecoratorWithoutLogin(eventService.removeFromFavorites, {
                    id: event.id,
                }).then((response) => {
                ...
                setIsFavorite(true);
                authDecoratorWithoutLogin(eventService.addToFavorites, {
                    id: event.id,
                }).then((response) => {
                    console.log(response);
                    if (response.status === 200) {
    ...
    return (
        <VisibleMainWrapper>
            <div
                className={`event-component ${!event ? 'loader' : ''}${
                    event && eventService.isOrganizer(event, user.user)
                        ? 'event-component-organizer'
                        : ''
                }`}
            >
                ...
                {event && (
                    <>
                        ...
                        {eventService.isHaveLimits(event) && (
                    ...
                    <VisibleEventOrganizer/> */}
                        ...
                        <span
                            className={`tooltip ${isTooltip ? '' : 'hidden'}`}
                        >
                            {tooltip}
                        </span>
                    </>
