    ...
    return (
        <div className={"board-event-component"}>
            {event && <>
                ...
                {event.time_end && <span className={"elem elem-3"}>{'Событие закончилось.'} {date.getDate()} {getMonth(date)} {date.getFullYear()}, {event.time_end.slice(0, 5)} {getWeekDay(date)}</span>}
