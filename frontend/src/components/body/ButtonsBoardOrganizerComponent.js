            ...
            {!event.is_end && <>
                {!event.cancel && <>
                    ...
                    {event.event_step.length === 1 && !event.event_step[0]["complete"] &&
                        <button className={`el el-${!event.is_begin ? '1' : '3'} btn-second`} onClick={toConfirmPlayers}>Подтверждение игроков</button>}
                    {event.event_step.length === 2 && !event.event_step[1]["complete"] &&
                        <button className={`el el-${!event.is_begin ? '1' : '3'} btn-second`} onClick={toFillRegulation}>Регламент</button>}
                    {event.event_step.length === 3 && !event.event_step[2]["complete"] &&
