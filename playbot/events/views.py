...
class CancelEventView(APIView):
    ...
    def post(self, request, format='json'):
        ...
        if serializer.is_valid() and event.organizers.filter(id=request.user.id).exists():
            event = serializer.save()
            if event:
                RankHistory.objects.create(user=request.user, rank=request.user.rank_fact * 0.98, reason=UserEventAction.Actions.CANCEL)
...
class LeaveEventView(APIView):
    ...
    def post(self, request, format='json'):
        ...
        if event_player.exists():
            event_player.delete()
            RankHistory.objects.create(user=request.user, rank=request.user.rank_fact * 0.99, reason=UserEventAction.Actions.LEAVE)
...
class AdminLeaveEventView(APIView):
    ...
    def post(self, request, format='json'):
        ...
        try:
            ...
            if event_player.exists():
                event_player.delete()
                RankHistory.objects.create(user=user, rank=user.rank_fact * 0.99, reason=UserEventAction.Actions.LEAVE)
                if event.first_order_queue:
                    EventPlayer.objects.update_or_create(player=event.first_order_queue, event=event)
                    EventQueue.objects.get(player=event.first_order_queue).delete()
            else:
